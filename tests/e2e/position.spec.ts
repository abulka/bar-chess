...
test('copies the LLM history with board, transcript, analysis and record', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write'])
  await page.evaluate(() => (window as any).game.loadSize(8))

  await page.getByRole('button', { name: 'Copy history for LLM' }).click()
  ...
  expect(llm).toContain('# transcript')
  expect(llm).toContain('# analysis')
  expect(llm).toContain('# replay record')
  expect(llm).toContain('partial')
})

test('copies a snapshot without the preamble', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write'])
  await page.evaluate(() => (window as any).game.loadSize(8))

  await page.getByRole('button', { name: 'Copy snapshot for LLM' }).click()
  const snapshot = await page.evaluate(() => navigator.clipboard.readText())
  expect(snapshot).toContain('# board-8 8x8')
  expect(snapshot).not.toContain('Bar Chess is a real-time')
})

test('copies the importable state as JSON', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write'])
  await page.evaluate(() => (window as any).game.loadSize(8))

  await page.getByRole('button', { name: 'Copy state (JSON)' }).click()

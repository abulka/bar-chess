...
- A piece best-efforts goto orders even to squares it cannot reach; once it is as close as it can get, your next order replaces the spent one instead of hiding behind it. A **standing attack order yields to self-preservation**: a hurt piece hesitates, walks to your king's healing aura, heals to about one hit above its retreat line, then resumes (a critical wound stays until fully healed). An attack on a target it can never hit is not chased at all. An **insta-kill** attack (see below) is the one thing that overrides this and is pressed regardless of wounds.
